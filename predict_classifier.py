...
class MyselfProcessor(DataProcessor):
  """Processor for the MRPC data set (GLUE version)."""
...
def predict(source="", target=""):
  ...
  if FLAGS.do_predict:
    ...
    if FLAGS.data_dir:
        ...
        guid = "%s-%s" % ("test", 0)
        label = "0"
        text_a = tokenization.convert_to_unicode(source)
        text_b = tokenization.convert_to_unicode(target)
    ...
    if FLAGS.output_dir:
        ...
        feature = convert_single_example(ex_index=0, example=predict_examples[0], label_list=label_list,
        ...
        predict_input_fn = input_fn_builder(features,
                                            seq_length=FLAGS.max_seq_length,
                                            is_training=False,
                                            drop_remainder=predict_drop_remainder)
